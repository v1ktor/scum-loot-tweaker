...
const breadcrumbMap: Record<string, string> = {
    ...
    [NavigationPath.MySpawners]: 'My Spawners',
...
export function NavBar({ content }: { content: ReactNode }) {
    ...
    return (
        <SidebarInset>
            <header className="flex h-14 shrink-0 items-center gap-2 sticky top-0 z-10 bg-background/95 backdrop-blur supports-backdrop-filter:bg-background/60">
                <div className="flex flex-1 items-center gap-2 px-3">
                    ...
                    <Breadcrumb>
                        <BreadcrumbList>
                            <BreadcrumbItem>
                                <BreadcrumbLink href="/">SCUM Loot Tweaker</BreadcrumbLink>
                            </BreadcrumbItem>
                            {currentPage && (
                                <>
                                    <BreadcrumbSeparator />
                                    <BreadcrumbItem>
                                        <BreadcrumbPage className="line-clamp-1">{currentPage}</BreadcrumbPage>
                                    </BreadcrumbItem>
                                </>
                            )}
                        </BreadcrumbList>
                ...
                <div className="ml-auto px-3">{false && <NavActions />}</div>
            </header>
            {content}
        </SidebarInset>
    );

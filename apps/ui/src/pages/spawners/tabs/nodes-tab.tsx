...
import {Spawner, SpawnerNode} from '@/pages/spawners/Spawners.types.ts';
...
export function NodesTab(props: NodesTabProps) {
  ...
  const syncToSpawner = useCallback((updatedRows: NodeGroup[]) => {
    setSpawner((prev) => ({
      ...prev,
      Nodes: updatedRows
        .map(({_uiId, ...node}) => ({
  ...
  const handleRarityChange = (nodeIndex: number, rarity: Rarity) => {
    setRows(prev => prev.map((node, i) =>
      i === nodeIndex ? {...node, Rarity: rarity} : node
    ));
  };
  ...
  const handleAddIdField = (nodeIndex: number) => {
    setRows(prev => prev.map((node, i) =>
      i === nodeIndex ? {...node, Ids: [...node.Ids, '']} : node
    ));
  };
  ...
  return (
    <div className="mt-4">
      <div className="flex justify-end py-4">
        <IconButton onClick={handleAddNode} leftOrnament={<PlusIcon className="mr-1 h-4 w-4"/>} text={"Add node group"}/>
      </div>
          ...
          {node.Ids.map((id, idIndex) => (
            <div key={`${node._uiId}-${idIndex}`} className="flex items-center gap-2 w-full">
              <Input
                ...
                onChange={handleUpdateId(nodeIndex, idIndex)}
              />
              {id && (
                  <IconButton
                    variant="ghost"
              ...
              <IconButton leftOrnament={<XIcon className="h-4 w-4"/>} variant="destructive" onClick={() => handleDeleteId(nodeIndex, idIndex)}/>
            </div>
          ))}

          <IconButton onClick={() => handleAddIdField(nodeIndex)} leftOrnament={<PlusIcon className="mr-1 h-4 w-4"/>} text={"Add node Id"} variant="outline"/>
        </Item>
      ))}
